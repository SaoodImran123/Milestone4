...
class PredictDoFn(beam.DoFn):
  ...
  def process(self, element, checkpoint):
    ...
    ts = int(element['key'])
    input_key = datetime.utcfromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
    image = element['image'].split(',')

    result = {input_key: image}
    return [result]
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    if known_args.source == 'text':
        ...
        consumer_config = {"topic": known_args.input,'bootstrap_servers':'pkc-6ojv2.us-west4.gcp.confluent.cloud:9092',\
            'security_protocol':'SASL_SSL','sasl_mechanism':'PLAIN','sasl_plain_username':'Y7C64F4YVMIKN6WF',\
            'sasl_plain_password':"WE9fIHhlW59aj47whmqrbzSxmVm472py3D9XOAVmAxf6Zc9vG3p1fp5Qu/wf8Ffy",\
